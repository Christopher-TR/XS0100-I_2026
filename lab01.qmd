---
title: "Laboratorio Estimación Puntual"
subtitle: |
...
set.seed(100000)
n <- 45
x <- rexp(n,rate=1/0.5) #R usa el argumento de "rate" como el inverso de beta.
hist(x)
mean(x)
var(x)
```
...
Volvemos al caso anterior con $n=10$. Calculo la variancia muestral para $k=1,...,K$ y obtengo la media y variancia (empírica) de la media muestral de las $K=1000$ medias muestrales.
...
```{r}
theta = 7
X <- runif(30, min = 0, max=theta)

K <- 1000
n <- 30
X <- list()
for(i in 1:K){
  X[[i]] <- runif(n, min = 0, max=theta)
}

#calcular el estimador insesgado para cada una de las muestras
